...
object WikipediaRanking extends WikipediaRankingInterface {
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(a => a.mentionsLanguage(lang)).count().toInt
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd
    .flatMap(a => langs.filter( l => a.mentionsLanguage(l)).map( l => (l, a)))
    .groupByKey()
   ...
   *   several seconds.
   */
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd
    .flatMap(a => langs.filter( l => a.mentionsLanguage(l)).map( l => (l, 1)))
    .reduceByKey(_+_)
    .sortBy(-_._2)
    .collect()
    .toList
