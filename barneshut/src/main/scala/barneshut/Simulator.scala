...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    res.minX = boundaries.minX
    res.minY = boundaries.minY
    res.maxX = boundaries.maxX
    res.maxY = boundaries.maxY

    if (body.x < res.minX) res.minX = body.x
    if (body.x > res.maxX) res.maxX = body.x
    if (body.y < res.minY) res.minY = body.y
    if (body.y > res.maxY) res.maxY = body.y
