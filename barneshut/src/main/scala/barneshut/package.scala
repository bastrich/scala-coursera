...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body])
    ...
    val (mass, massX, massY) = {
      ...
      if (mass == 0) {
        (mass, centerX, centerY)
      } else {
        (
          mass,
          bodies.map(b => b.mass * b.x).sum / mass,
          bodies.map(b => b.mass * b.y).sum / mass
        )
      }
    }
    ...
    def insert(b: Body): Quad = if (size > minimumSize) {
      ...
      bodies.foldLeft(res)((q, b) => q.insert(b)).insert(b)
    } else {
  ...
  class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float) {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        case Empty(_, _, _) =>
        case Leaf(_, _, _, bodies) => bodies
          .filterNot(_ == this)
          .foreach(b => {
            addForce(b.mass, b.x, b.y)
          })
        case f @ Fork(nw, ne, sw, se) =>
          if (f.size / distance(x, y, f.centerX, f.centerY) < theta) {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    private def getX(index: Int): Float = (index % sectorPrecision) * sectorSize + boundaries.minX + sectorSize / 2

    private def getY(index: Int): Float = (index / sectorPrecision) * sectorSize + boundaries.minY + sectorSize / 2


    def +=(b: Body): SectorMatrix = {
      val closestCB = matrix.zipWithIndex.minBy {
        case (zb, index) => distance(b.x, b.y, getX(index), getY(index))
      }((x: Float, y: Float) => if (x <= y) -1 else 1)._2
      matrix(closestCB) = matrix(closestCB) += b
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      for (cbPair <- matrix.zip(that.matrix).zipWithIndex) {
        res.matrix(cbPair._2) = cbPair._1._1.combine(cbPair._1._2)
      }
      res
    }
