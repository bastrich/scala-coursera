...
class KMeansSuite {
  ...
  @Test def `test`: Unit = {
    val points: Seq[Point] = Seq(new Point(0, 0, 1), new Point(0,0, -1), new Point(0,1,0), new Point(0,10,0))
    val means: Seq[Point] = Seq(new Point(0, -1, 0), new Point(0, 2, 0))
    val expected: Seq[Point] = Seq(new Point(0.0, 0.0, 0.0), new Point(0.0, 5.5, 0.0))

    assertEquals(expected, kMeans(points, means, 12.25))
  }
